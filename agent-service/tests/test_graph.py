...
class GraphTests(unittest.TestCase):
    ...
    def test_graph_restores_session_between_requests(self):
        ...
        s=self.client.post('/decide',headers=self.headers,json={'operation':'turn','session':s,
            'payload':{'text':'2000','event_id':'answer','revision':1}}).json()
        self.assertEqual(s['pending']['value'],'2000')
        self.assertFalse(s['fields'])
    ...
    def test_persisted_receipt_is_reused(self):
        state=graph.invoke({'request':Decision(operation='start',payload={'seed':{'postcode':'2000','fuel':'gas','occupancy':'own','solar':False,'moving':False}})})['session']
