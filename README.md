...
5. Use Speak / interrupt for browser speech input. Spoken replies use the same pacing and cancellation logic as the reference.
...
The unit suite inherits the reference's 59 policy/script/telephony tests, adapts persistence tests for request-local state, and adds graph transport/authentication tests. PostgreSQL durability is covered by the HTTP smoke script. Java tests cover webhook signatures. The frontend runs five voice timing/cancellation tests and a production build. The live test confirms the complete HTTP → graph → model → PostgreSQL handover path.
